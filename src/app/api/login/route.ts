...
export async function POST(req: NextRequest) {
  ...
  if (!ADMIN_PASSWORD) return NextResponse.json({ message: "ADMIN_PASSWORD belum diset" }, { status: 500 });
  ...
  res.cookies.set("admin_auth", "1", { httpOnly: true, secure: true, sameSite: "lax", path: "/" });
  return res;
}
